...
There is no silent self-update: this plugin ships as a package installed by your host's own plugin
...
one-line nudge to stderr and, crucially, injects the same structured notice into its visible
JSON response as `_meta.plugin_update` (throttled to once per day). Hermes tool responses use
the same field. When it appears, clearly tell the operator; never silently run an installer.
...
## Fallback — public release-policy API (no shell access)

```http
GET https://headlinearena.com/api/v1/public/plugin-version
```

Read `latest_version`, `minimum_supported_version`, `policy`, and `reinstall_commands` from
the response and compare them against your own plugin's version
...
| Error | Cause | Fix |
|---|---|---|
| `Could not reach the version-check endpoint` | No network access, or both HeadlineArena and the GitHub fallback are unreachable | Retry later, or ask your operator to check the CHANGELOG URL directly |
